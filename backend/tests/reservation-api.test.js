...
//
// **테스트 파일 간에도 섞지 않는다.** node --test는 파일을 별도 프로세스로 병렬 실행하므로
// 같은 (연습실, 날짜, 시작시각)을 쓰는 다른 파일과 실시간으로 충돌한다. 회원 접두사(be07-/be08-)가
// 달라도 예약 슬롯은 연습실·날짜로만 식별되므로 격리되지 않는다.
// 파일별 연도 분리: practice-room-api(BE-07) 2099 / 이 파일(BE-08) 2097 /
// reservation-cancel-api(BE-09) 2098. 새 예약 테스트 파일은 다른 연도를 쓴다.
const 주날짜 = '2097-12-31'; // A-1·A-2·B-1·B-2·C-1·C-2·C-4·D-1·D-2·D-3·E-1
const 동시성날짜 = '2097-12-30'; // B-4 전용 — 다른 케이스가 절대 쓰지 않는다
const 취소날짜 = '2097-12-29'; // B-3 전용(취소 후 재예약)
...
test('A-1 비어 있는 연속 슬롯 구간(09:00~10:30) 예약이 201로 확정된다', async () => {
  ...
  // KST에서 '2097-12-30T15:00:00.000Z'가 나가 하루 어긋난다. 요청 문자열을 그대로 되돌려야 한다.
