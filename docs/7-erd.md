...
## 1. 문서 개요
도메인 정의서(1-domain-definition.md v0.6)와 프로젝트 구조 설계 원칙(5-project-principle.md v0.4)을 기반으로, PostgreSQL 17에 순수 SQL(CREATE TABLE)로 바로 옮길 수 있는 물리 데이터 모델을 정의한다.
...
| `boards` | 게시판 정의, 등급별 접근 제한 | `min_grade_level` FK → `member_grades(grade_level)` ON DELETE RESTRICT, `is_active` NOT NULL DEFAULT true | PK 인덱스로 충분(게시판 수가 적음) |
