...
| 0.22 | 2026-09-11 | 실제 배포 형태 반영. §5 배포/운영이 "단일 서버"를 단정하고 있었으나 운영은 서버리스 + 관리형 DB(Vercel·Supabase)로 올라가 있어 어긋났다. 기본 형태는 그대로 두고 실제 형태와 그때 달라지는 것(플랫폼 환경변수·재배포, **SPA 폴백을 호스팅으로 옮겨야 함**, 트랜잭션 모드 풀러)을 하위 항목으로 붙였다. "단일 서버"가 규모를 키우지 않겠다는 뜻이지 호스팅을 강제하는 것이 아님도 명시 | Kang SangSoo |
| 0.21 | 2026-09-11 | 문서 간 정합성 검토 반영. §4의 "브라우저 UI E2E는 범위 외"를 **"저장소가 관리하지 않는다"**로 다시 적었다 — 하지 않는다는 뜻으로 읽히면, 로컬에서 E2E를 돌린 사실과 어긋나고 `test/`를 버려진 디렉토리로 오해하게 된다. 이에 따라 "러너는 스택별로 하나씩"이 세는 대상이 저장소가 관리하는 둘(`node --test`, Vitest)임도 명시 | Kang SangSoo |
...
- 성능/부하 테스트와 접근성 테스트는 범위 외 (PRD §3 Out of Scope, §9 가정사항과 일치). 시나리오 E2E는 백엔드 API에 한해 `backend/tests/e2e-scenarios.sh`로 자동화돼 있고(S-01~S-08), **브라우저를 구동하는 UI E2E는 이 저장소의 범위 밖이다.**
  - 범위 밖이라는 것은 "하지 않는다"가 아니라 **저장소가 관리하지 않는다**는 뜻이다. 필요하면 각자 로컬에서 도구를 골라 돌리되, 그 스펙·설정·리포트·스크린샷은 커밋하지 않는다(`.gitignore`의 `test/`). 대상이 떠 있는 개발서버와 개발 DB라 결과가 실행 환경에 따라 달라지고, 저장소 안에서 완결되지 않기 때문이다.
  - 따라서 위 "러너는 스택별로 하나씩" 규칙이 세는 대상은 **저장소가 관리하는 러너 둘**(`node --test`, Vitest)이다. 로컬 전용 도구는 이 셈에 들어가지 않는다.
...
- **배포/운영**: 기본 형태는 단일 서버(예: 단일 VM/컨테이너)에 Express 앱 + PostgreSQL이다. 별도 로드밸런서/이중화는 두지 않는다(PRD §5 가용성 요구사항과 일치). 별도 로그 수집 인프라(ELK 등)는 두지 않는다.
  - **실제 운영은 서버리스 + 관리형 DB 형태로 배포되어 있다**(프론트·백엔드 각각 Vercel, DB는 Supabase). 절차와 주의사항은 `backend/README.md` §배포 (다)에 있다. 코드는 그대로 쓰지만 운영 방식이 몇 가지 달라진다.
    - 환경변수를 `.env` 파일이 아니라 플랫폼 설정에서 읽고, 값을 고치면 재배포해야 반영된다.
    - **SPA 폴백을 호스팅 쪽에 따로 설정해야 한다.** 단일 서버에서는 `STATIC_DIR` 폴백이 하던 일이라, 이 형태로 옮기면 빠뜨리기 쉽다 — 빠뜨리면 `/` 외 모든 주소가 404가 된다.
    - 프로세스가 요청마다 뜨고 사라지므로 커넥션 풀을 프로세스가 오래 들고 있다는 전제가 깨진다. 관리형 DB의 **트랜잭션 모드 풀러**를 경유해 커넥션 고갈을 막는다.
    - "단일 서버"를 전제로 쓰인 서술(이 항목, PRD §5 가용성)은 **규모를 키우지 않겠다는 뜻**이지 특정 호스팅을 강제하는 것이 아니다. 이중화·오토스케일을 우리가 설계하지 않는다는 원칙은 이 형태에서도 그대로다.
...
│   │   ├── practiceRoom/
│   │   │   ├── reservationRules.ts  # 취소 가능 판정·상태 문구 (순수 함수)
